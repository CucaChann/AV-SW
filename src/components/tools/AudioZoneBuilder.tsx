          ...
          return (
            <article className="builder-card" key={zone.id}>
              ...
              <div className="builder-form-grid three">
                ...
                <label className="field"><span>Amplification</span><select value={zone.amplification} onChange={(e) => updateZone(zone.id, { amplification: e.target.value as AudioZone["amplification"] })}>{["DSP / Multi-Channel Amp","AVR / Marantz","Powered Speaker","Sonos Amp","TBD"].map((v) => <option key={v}>{v}</option>)}</select></label>
                <label className="field"><span>Client control</span><select value={zone.control} onChange={(e) => updateZone(zone.id, { control: e.target.value as AudioZone["control"] })}>{["Savant","Native App","Mixed","Sonos","TBD"].map((v) => <option key={v}>{v}</option>)}</select></label>
              </div>
